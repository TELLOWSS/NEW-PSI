...
- api/harness/workflow-status.ts가 persistence 연결은 되었지만 저장 런이 없는 경우 별도 경고와 진단 메타를 응답해 `폴백`과 `실데이터 미발견`을 구분 가능하게 정리
...
- api/harness/persistence-health.ts와 services/harnessService.ts에 하네스 persistence 환경 상태 조회 엔드포인트/클라이언트 래퍼를 추가
