...
## 🔎 2026-04-16 OCR 정밀 역추적 업데이트 (하네스 격상 이후)
...
2. API 키 선택 폴백 강화
     - 현재 모드 키가 없으면 반대 모드 키(local/env)를 순차 폴백하도록 개선
3. 모델 폴백 강화
     - 모델 가용성 오류 시 기존 fallback 모델 이후 `gemini-2.5-flash` 안정 폴백 추가
4. 할당량 오류 감지 일관화
     - 루프 내부/최종 에러 변환 모두 공통 `isRateLimitError()` 기준 사용

### 운영 검증 체크리스트 (즉시 실행)
- [ ] 동일 원본 이미지 10건으로 재분석 시 손상 오탐 0건 확인
- [ ] 무료 모드 ON + 무료키 공란 + 유료키 존재 상태에서 실제 분석 성공 여부 확인(키 폴백 검증)
