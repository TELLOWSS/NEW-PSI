...
## 🔎 2026-09-08 OCR 모델 수명주기·비용 통제 업데이트

### 현재 운영 원칙
- 무료 모드는 `gemini-3.5-flash-lite`를 우선 사용하고, 가용성 오류일 때만 무료 키 범위에서 `gemini-3.8-flash`로 폴백한다.
- 정밀 모드는 `gemini-3.8-flash`를 사용한다. 유료 키 호출에는 관리자 비밀번호 승인으로 발급된 1회용 토큰이 반드시 필요하다.
- 무료 키에서 유료 키로 자동 전환하지 않으며, 비용 산정 실패·미등록 모델·상한 초과는 모두 호출 전에 차단한다.
- 공급자 요청은 Interactions API와 `store: false`를 사용한다.

## 🔎 2026-04-16 OCR 정밀 역추적 업데이트 (하네스 격상 이후, 변경 이력)

### 결론 요약
- 1차 원인: 이미지 손상 오탐(False Positive)
    - 기존 `validateImageFormat()`이 base64 본문에 `null`, `undefined`, `NaN` 문자열이 포함되면 손상으로 즉시 판정하던 규칙이 정상 이미지에도 오탐을 유발함.
- 2차 원인: 무료/유료 모드 전환 시 키 선택 취약
    - 현재 모드 키가 비어 있으면 반대 모드 키를 사용하지 못해, 실제 키가 있어도 실패 체감이 발생할 수 있었음.
- 3차 원인: 모델 가용성 변화 대응 부족
    - 1차/2차 모델 모두 가용하지 않을 때 안정 모델로 자동 전환하는 단계가 부족했음.
- 하네스 영향 범위
    - 하네스는 OCR 이미지 바이트 검증 엔진이 아니라 OCR 결과 텍스트 기반 검증/승인 워크플로우 계층이며, 이미지 손상 오탐의 직접 원인은 아님.

### 코드 반영 사항 (2026-04-16)
1. 이미지 손상 판정 로직 개선
     - 문자열 휴리스틱(`null/undefined/NaN`) 제거
     - Base64 URL-safe 문자(`-`, `_`) 정규화 + 패딩 보정 + decode 기반 무결성 검증으로 교체
2. API 키 선택 폴백 강화(당시 이력)
     - 현재 정책에서는 반대 모드 키 자동 폴백을 폐지하고 무료/유료 키를 엄격히 분리함
3. 모델 폴백 강화
     - 종료된 `gemini-2.5-flash`를 제거하고 현행 GA 모델 체인으로 교체함
4. 할당량 오류 감지 일관화
     - 루프 내부/최종 에러 변환 모두 공통 `isRateLimitError()` 기준 사용

### 운영 검증 체크리스트 (즉시 실행)
- [ ] 동일 원본 이미지 10건으로 재분석 시 손상 오탐 0건 확인
- [ ] 무료 모드 ON + 무료키 공란 + 유료키 존재 상태에서 유료키가 사용되지 않는지 확인
- [ ] 모델 가용성 오류 시 자동 폴백 로그 확인
- [ ] 429 발생 후 냉각/재시도 메시지 일관성 확인
- [ ] 하네스 decision은 생성되지만 OCR 실패 유형은 별도 집계되는지 확인

### 신규 관측 지표 (2026-04-16)
- 실패코드 대시보드에 최근 24시간 갱신 실패 건수/비율 추가
- 집중 코드(Top failure code) 자동 표시로 정책 변화 징후(QUOTA 급증 등) 조기 탐지
- 운영 요약 내보내기 텍스트에 [실패코드 최근 24시간] 섹션 추가
...
1. ✅ **기본 유효성** - `validateImageFormat()`
   - 데이터 길이 확인
    - URL-safe base64 정규화(`-`,`_`) + 패딩 보정
    - decode 기반 무결성 확인(오탐 최소화)
   - Base64 정규식 검증
