    ...
    return (        <div className="bg-slate-100 min-h-screen p-3 sm:p-6 flex flex-col items-center gap-4 sm:gap-6 pb-20 no-print font-sans">
            ...
            <section className="w-full max-w-[210mm] rounded-2xl border border-slate-200 bg-white p-5 font-sans shadow-sm">
                <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                        <p className="text-[11px] font-black uppercase tracking-[0.18em] text-indigo-600">관리자 분석</p>
                        <h3 className="mt-1 text-lg font-black tracking-tight text-slate-900">개인 작성 경향 분석</h3>
                        <p className="mt-1 text-xs font-bold leading-relaxed text-slate-500">개인 공개용이 아닌 관리자 개선 추적 자료입니다.</p>
                    </div>
                    <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-black text-slate-600">{adminInsightReport.assessmentMonth || '-'} · {adminInsightReport.workType}</span>
                </div>
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                    <div className="rounded-xl border border-amber-100 bg-amber-50 p-4">
                        <p className="text-xs font-black text-amber-700">반복 확인 항목</p>
                        <p className="mt-2 text-sm font-bold leading-relaxed text-slate-700">{adminInsightReport.repeatedIssues.join(' · ') || '확인된 반복 항목 없음'}</p>
                    </div>
                    <div className="rounded-xl border border-emerald-100 bg-emerald-50 p-4">
                        <p className="text-xs font-black text-emerald-700">개선이행 확인</p>
                        <p className="mt-2 text-sm font-bold leading-relaxed text-slate-700">{adminInsightReport.improvementActions.join(' · ') || '관리자 확인 후 개선행동을 등록하세요.'}</p>
                    </div>
                </div>
            </section>
