...
export default function LocalBackupArchivePanel() {
    ...
    const execute = async (action: () => Promise<void>) => {
        ...
        working.current = true; setBusy(true); setRecord(undefined); setShowImage(false);
        try { await action(); }
    ...
    return <details className="rounded-2xl border border-sky-300/40 bg-slate-950 p-4 text-slate-100">
        ...
        <div className="mt-3 space-y-3 text-sm leading-relaxed">
            <p>월별 백업을 기록별 JSON과 가벼운 목록으로 저장합니다. 목록만 읽고 원문은 1건씩 엽니다. 서버 전송·유료 OCR·운영 DB 변경 없음.</p>
