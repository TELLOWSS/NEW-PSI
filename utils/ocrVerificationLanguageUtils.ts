...
export const getNativeLanguageLabel = (nationality: string, language?: string): string => {
    switch (inferNativeLangCode(nationality, language)) {
        case 'ko': return '한국어';
        case 'vi': return '베트남어';
        case 'zh': return '중국어';
        case 'th': return '태국어';
...
export const buildFallbackNativeVerdictText = (record: Pick<OcrVerificationLikeRecord, 'nationality' | 'language' | 'jobField' | 'weakAreas' | 'aiInsights'>): string => {
    ...
    if (langCode === 'vi') return `Bản đánh giá này cho thấy trong công việc ${job}, cần kiểm tra lại biện pháp bảo vệ đối với ${weak || 'nguy cơ chính'}. ${koreanSummary}`;
    if (langCode === 'zh') return `本次判断说明在${job}作业中，需要再次确认针对${weak || '主要风险'}的防护措施。${koreanSummary}`;
    if (langCode === 'th') return `ผลการประเมินนี้ชี้ว่าในงาน${job} ต้องตรวจสอบมาตรการป้องกันสำหรับ${weak || 'ความเสี่ยงหลัก'}อีกครั้ง ${koreanSummary}`;
    if (langCode === 'uz') return `Ushbu tahlil ${job} ishida ${weak || 'asosiy xavf'} bo'yicha himoya choralarini yana bir bor tekshirish kerakligini ko'rsatadi. ${koreanSummary}`;
    if (langCode === 'id') return `Penilaian ini menunjukkan bahwa dalam pekerjaan ${job}, tindakan perlindungan terhadap ${weak || 'risiko utama'} perlu diperiksa kembali. ${koreanSummary}`;
    if (langCode === 'km') return `ការវាយតម្លៃនេះបង្ហាញថា ក្នុងការងារ ${job} ត្រូវពិនិត្យឡើងវិញនូវវិធានការការពារសម្រាប់ ${weak || 'ហានិភ័យសំខាន់'}។ ${koreanSummary}`;
    if (langCode === 'mn') return `Энэ дүгнэлтээр ${job} ажилд ${weak || 'гол эрсдэл'}-ийн хамгаалалтын арга хэмжээг дахин шалгах шаардлагатай байна. ${koreanSummary}`;
    if (langCode === 'kk') return `Бұл талдау ${job} жұмысында ${weak || 'негізгі тәуекел'} бойынша қорғаныс шараларын қайта тексеру қажет екенін көрсетеді. ${koreanSummary}`;
    if (langCode === 'ru') return `Данная оценка показывает, что в работе ${job} нужно повторно проверить меры защиты по ${weak || 'основному риску'}. ${koreanSummary}`;
    if (langCode === 'ne') return `यो मूल्यांकनले ${job} काममा ${weak || 'मुख्य जोखिम'} सम्बन्धी सुरक्षा उपाय फेरि जाँच गर्न आवश्यक देखाउँछ। ${koreanSummary}`;
    if (langCode === 'my') return `ဤသုံးသပ်ချက်အရ ${job} အလုပ်တွင် ${weak || 'အဓိကအန္တရာယ်'} အတွက် ကာကွယ်ရေးအစီအမံကို ထပ်မံစစ်ဆေးရန် လိုအပ်ပါသည်။ ${koreanSummary}`;
