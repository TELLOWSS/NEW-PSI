...
export const buildExternalAiPrompt = (options: {
    ...
    if (options.draft) {
        lines.push(
            ...
            '5. 번역본(translations) 내부에는 한국어나 영어 단어(예: "TBM", "seconds", "accident" 등)가 그대로 섞여 나오지 않고, 100% 해당 번역 대상 국가의 공식 모국어 문자와 자연스러운 어조로 완전히 번역되도록 해주십시오. 전문 용어 또한 현지 외국인 근로자가 완벽히 이해할 수 있는 모국어 표현으로 전환해 번역하십시오.',
            '6. 각 단계의 머리말 레이블(예: "1. 교육 전 5분 핵심 동영상", "2. 최근 재해사례와 현장 연관성", "[이해 확인 및 행동 약속]" 등) 또한 해당 모국어로 완벽하게 번역해야 합니다.',
            ''
        );
    }
    ...
    lines.push(
        ...
        '8. 번역은 한국 건설현장 용어의 의무 강도를 유지하고 1~5단계 구조를 보존하되, 한국어나 영어가 번역 텍스트에 섞이지 않고 지정된 모국어로만 출력되게 하십시오.',
        '',
        '[응답 형식]',
