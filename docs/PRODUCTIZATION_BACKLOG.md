...
### [x] P1 지표 규칙 버전·단일 계산 기준

공통 운영 지표를 `psi-core-metrics-2026-06-22-v1`로 고정하고 공식 지표 카탈로그와 단일 계산 모듈을 적용했다.

- 현재 지표는 근로자별 최신 유효 기록 1건만 사용
- 월 추세는 근로자별·월별 최신 기록 1건만 사용
- 보호 우선은 저장된 등급 문자열이 아니라 현재 점수 임계값으로 판정
- 통합 보드·대시보드·성과 분석·월 추세 차트·리포트 계산 기준 통합
- 정적 계산 소유권 검사와 단위·브라우저 회귀검사 추가
- 상세 기준: `docs/METRIC_CATALOG.md`
...
2. 관리자 RBAC·현장/테넌트 격리 및 Supabase RLS 회귀검사
3. 대용량 서버 스트리밍 복원·중단 재개·원자적 롤백
4. 현장×월×공종×세부작업 기준 모델
