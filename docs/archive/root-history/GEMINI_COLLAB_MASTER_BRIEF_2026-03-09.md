...
- 최근 반영(이번 세션): 피드백 Webhook/Outbox 고도화, 보고서 진행률 퍼센트 UI(개별/일괄), OCR 실패 원인 분류 및 재촬영 UX, 발급 신뢰성 게이트, 다국어 교육(관리자 생성/근로자 서명) 및 설정 기반 언어 프리셋 연동, Vercel 배포 안정화(Supabase 환경변수 로딩/초기 렌더 생존성 강화)
...
### E. 배포/운영 안정화 핫픽스 (2026-03-10)
- Vercel 배포 설정 고정: `vercel.json`에 빌드 명령/출력 디렉터리(`dist`) 명시
- 초기 렌더 생존성 강화: 앱 페이지 지연 로딩(`React.lazy`, `Suspense`) 적용
- 앱 부트스트랩 오류 가시화: 무반응 공백 대신 초기화 오류 화면 표시
- LocalStorage 손상 데이터 방어: `psi_safety_checks` 파싱 실패 시 안전 fallback
- Supabase 환경변수 로딩 안정화:
	- Vite에서 `NEXT_PUBLIC_*` 접두어도 허용(`envPrefix`)하도록 설정
	- `lib/supabaseClient.ts`에서 `VITE_*`/`NEXT_PUBLIC_*` 동시 지원
	- 누락 시 즉시 원인 메시지를 노출해 배포 진단 시간 단축
...
- 관리자 다국어 교육(신규): 한국어 원문 기반 다국어 TTS 생성 및 QR 공유
- 근로자 교육/서명(신규): 모바일 접속 후 서명 제출 및 이력 저장
